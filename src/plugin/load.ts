...
import { fixMapSources, extractSource } from '../utils/extractSource.js';
import { buildWorkerCode } from '../utils/buildWorkerCode.js';

const HELPER_PATTERN = /^\0(?:[0-9]+::)?rollup-plugin-web-worker-loader::helper(?:::)?/;
...
function updateWatchModules(modules: Record<string, unknown>, addWatchFile: (id: string) => void): void {
...
function handleBundleGenerated(
    ...
    if (config.inline || effectivePlatform === 'base64') {
        ...
        if (config.sourcemap && chunk.map) {
            map = fixMapSources(
                { map: chunk.map as Parameters<typeof fixMapSources>[0]['map'], modules: chunk.modules ?? {} },
                state.basePath ?? '.',
            );
        }
    } else {
...
async function bundleWorker(
    ...
    let bundler: { rollup: (options: object) => Promise<RollupBundle> } | null = null;
    let bundlerName = '';

    try {
        bundler = await import('rolldown') as { rollup: (options: object) => Promise<RollupBundle> };
    ...
    if (!bundler) {
        try {
            bundler = await import('@rolldown/rolldown') as { rollup: (options: object) => Promise<RollupBundle> };
    ...
    if (!bundler) {
        try {
            bundler = await import('rollup') as { rollup: (options: object) => Promise<RollupBundle> };
            bundlerName = 'rollup';
        } catch {
    ...
    try {
        ...
        for (const [key, val] of state.idMap) {
            if (val.target === target) {
                return handleBundleGenerated(state, config, addWatchFile, key, workerID, result);
            }
        }
        return null;
    } catch (reason) {

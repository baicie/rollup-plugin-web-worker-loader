...
type RolldownChunk = OutputChunk & { type: 'chunk' };
type RollupChunk = { code?: string; map?: unknown; modules?: Record<string, unknown>; fileName?: string };
...
export function handleGenerateBundle(
    ...
    if (!config.inline && isWrite) {
        if (state.configuredFileNames.size > 0 && Object.keys(bundle).length === 1) {
            ...
            const configuredName = state.configuredFileNames.get((bundle[key] as { facadeModuleId?: string }).facadeModuleId?.includes('.css') ? 'es' : 'es');
            if (configuredName) {
                (bundle[key] as { fileName?: string }).fileName = configuredName;
            }
        }
        for (const [id, worker] of state.idMap) {

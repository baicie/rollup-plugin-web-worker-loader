...
export function handleOptions(state: InternalState, config: InternalConfig, options: RollupInputOptions): null {
    if (!state.options) {
        state.options = { ...options };
        if (options.plugins && options.plugins.length) {
            const plugins: unknown[] = [];
            for (const plugin of options.plugins) {
                if (typeof plugin === 'object' && plugin && 'name' in plugin && config.skipPlugins.has((plugin as { name: string }).name)) {
                    continue;
                }
                plugins.push(plugin);
            }
            (state.options as RollupInputOptions & { plugins: unknown[] }).plugins = plugins;

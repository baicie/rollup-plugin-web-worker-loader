...
export function findFullPath(file, length, modules) {
...
export function fixMapSources(chunk, basePath) {
  const map = chunk.map
  ...
  for (let i = 0; i < map.sources.length; ++i) {
    const full =
      ...
      findFullPath(map.sources[i], map.sourcesContent[i].length, chunk.modules)
    if (full) {
